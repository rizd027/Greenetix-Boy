            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 max-w-6xl mx-auto">
                    {galleryData.items.map((item, index) => (
                        <div key={item.id} className="group relative rounded-2xl md:rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1">
                            <div className="relative w-full h-48 md:h-80">
                                <Image
                                    ...
                                    {...(index === galleryData.items.length - 1 ? { sizes: "(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" } : {})}
                                />
                            </div>
                            <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4 md:p-6">
                                <p className="text-white font-black text-xs md:text-lg uppercase tracking-wider">{item.title}</p>
                            </div>
                        </div>
                    ))}
                </div>
